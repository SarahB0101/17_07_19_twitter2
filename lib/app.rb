...
def random
	...
	@table_journalists << @journalists.sample(5) 
	puts @table_journalists
	return @table_journalists
...
def tweet_sentance (client)
	
	table_journalists = random
	#table_journalists = ["@thpslacker"] # Uniquement pour faire des tests d'envoi.
	table_journalists.each {|journalist_n| client.update("Bonjour #{journalist_n} je vous tweet depuis Ruby. Je suis en pleine formation avec TheHackingProject.org !")} 
	 
	
end

tweet_sentance(client)
